...
public class ProtocolTask implements Runnable {
    ...
    /**
     * A protocol task remains alive as long as the connection is alive.
     * It is reused each time new work is detected by the reactor and passed to the thread pool executor.
     * The same task can be queued several times into the thread pool, in reaction to several events.
     * Therefore, a task can be executed concurrently by several threads.
     *
     * @param protocol instance of the protocol of the specific connection.
     * @param tokenizer instance of the tokneizer of the specific connection - may contain partially decoded bytes.
     * @param h connection handler that requires this task to be executed.
     */
    ...
    /**
     * Task: read as much as possible from incoming channel.
     *       then parse all complete messages and pass them to protocol instance.
     * Synchronize on ourselves, in case we are executed by several threads
