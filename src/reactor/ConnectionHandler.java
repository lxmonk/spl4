...
public class ConnectionHandler implements Sender {
	 ...
	 */
	private ConnectionHandler(SocketChannel sChannel, ReactorData data,
			SelectionKey key) {
		_sChannel = sChannel;
		_data = data;
	...
	/**
	 * Static factory method (ctor is private). This way we make sure reference
	 * to this does not escape through the selector before the connectionHandler
	 * is fully constructed.
	 * 
	 * @param sChannel
	 *            non-blocking socket channel just accepted from server socket.
	 * @param data
	 *            all Reactor data needed by handler.
	 * @param key
	 *            selector key to which sChannel is connected.
	 * @return fully constructed connectionHandler.
	 */
	public static ConnectionHandler create(SocketChannel sChannel,
			ReactorData data, SelectionKey key) {
	...
	/**
	 * Send bytes to the connected client. Selector will remain in Write mode
	...
	/**
	 * Frame and send a string to client Invoked by protocol when it needs to
	 * send to various clients.
	 * 
	 * @param s
	 *            string to be framed (not encoded, no delimiter)
	 */
